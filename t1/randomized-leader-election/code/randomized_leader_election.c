...
typedef struct {
    ...
    int timeout;          // numero de mensagens que faltam ate receber a propria mensagem de novo
    int alive;            // numero de processos vivos 
            ...
            case RECEIVE:
                ...
                if (p->timeout == 0) {
                    if (p->msg.sender_id == p->pid) {
                        ...
                        printf("       O processo %d detectou uma falha no sequenciamento: reiniciando bit\n", p->pid);
